...
- 在名单中：发送替换后的文本，以及标有对应 Image ID 的 `image_url` 内容块。
...
后端能访问图片不代表供应商也能访问；供应商下载图片失败或配置的模型拒绝图片，
仍按原有模型调用失败返回 HTTP 502，不静默重试或降级。

返回给 OCS 的 `question` 保留原题干。答案中的已知 Image ID 恢复为原始图片 URL，
字母编号与其他答案文字保持原样；`analysis` 和路由日志中的原始模型答案不改写。
